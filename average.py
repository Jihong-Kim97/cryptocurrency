...
file_directory = "C:/Users/KimJihong/Desktop/김지홍/개발/코인/DB/{}{}{}_average_{}_{}.csv".format(now_year,now_month,now_day,allowable_loss,target_profit)
...
for date in tqdm(dates, desc='backtesting 중...'):
    ...
    for ticker in tickers:
        ...
        if flag_inverted_hammer[ticker] == True and ticker != target_ticker:  
            ...
            if flag_dip[ticker] == True:           
                if ror > 0 and close > curr_ma224[ticker]: #and volume_ratio < 5
                    flag_buy[ticker] = 1
                    price[ticker] = close
        ...
        if target_ticker == ticker and flag_hold and trading_day > 10:
            ...
            ##########이평선 이탈#############
            if close < curr_ma224[ticker] and flag_sell == False:
                flag_sell =  True
    ...
    if flag_hold == False:
        ...
        if len(candidates) > 0 and flag_activity == False:
            ...
            print("ma_224: {}".format(curr_ma224[target_ticker]))
    else:
        candidates = [t for t,v in flag_buy.items() if v > 0]
        if len(candidates) > 0:
            inverted_hammer_candidate = {}
